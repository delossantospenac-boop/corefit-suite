...
function MisClases() {
  ...
  const now = Date.now();
  const upcoming = (data ?? []).filter((a) => new Date(a.starts_at).getTime() >= now && a.status !== "cancelada");
  const past = (data ?? []).filter((a) => new Date(a.starts_at).getTime() < now || a.status === "cancelada");
